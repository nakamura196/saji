...
class PixPlot:
  def __init__(self, image_glob):
    ...
    with open('data/002_err.csv', 'w') as f:
      writer = csv.writer(f)
    ...
    with tf.Session() as sess:
      for image_index, image in enumerate(self.image_files):
        try:
          ...
          if DELETE_FLAG:
            os.remove(get_ascii_chars(image))
            id = get_ascii_chars(image).split("/")[-1].split(".")[0]
            rows.append([id])
